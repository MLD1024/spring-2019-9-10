 ...
 * 两者的用法以及对注解的处理方式几乎没有什么差别 通过分析这个类我们知道注册一个bean到spring容器有两种办法
 *  一、直接将注解Bean注册到容器中：（参考）public void register(Class<?>... annotatedClasses)但是直接把一个注解的bean注册到容器当中也分为两种方法
 *  	1、在初始化容器时注册并且解析
...
public class AnnotationConfigApplicationContext extends GenericApplicationContext implements AnnotationConfigRegistry {
	...
	/**
	 * 这个类顾名思义是一个reader
	 * 还是顾名思义AnnotatedBeanDefinition意思是读取一个被加了注解的bean
	 * 这个类在构造方法中实例化的
	 */
	...
	/**
	 * 这个构造方法需要传入一个被javaconfig注解了的配置类<p>
	 * 然后会把这个被注解了javaconfig的类通过注解读取器读取后继而解析<p>
